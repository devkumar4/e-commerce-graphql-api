...
describe('Order Integration', () => {
  ...
  beforeAll(async () => {
    ...
    // Login as admin
    const resAdmin = await request(server.httpServer).post('/graphql').send({
      query: `
        mutation {
    ...
    const resCustomer = await request(server.httpServer).post('/graphql').send({
      query: `
        mutation {
  ...
  it('queries orders as customer', async () => {
    ...
    expect(res.body.data.orders.some((o: any) => o.id === orderId)).toBe(true);
  });
